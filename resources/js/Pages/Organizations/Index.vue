      ...
      <table class="w-full whitespace-no-wrap">
        <tr class="text-left font-bold">
          <th class="px-6 pt-6 pb-4">
            <div class="inline-flex items-center cursor-pointer" @click="orderBy('name')">
              <span>Name</span>
              <icon class="w-4 h-4 fill-grey-darker ml-1" v-if="form.order.field === 'name'" :name="form.order.direction === 'asc' ? 'arrow-up' : 'arrow-down'"/>
            </div>
          </th>
          <th class="px-6 pt-6 pb-4">
            <div class="inline-flex items-center cursor-pointer" @click="orderBy('city')">
              <span>City</span>
              <icon class="w-4 h-4 fill-grey-darker ml-1" v-if="form.order.field === 'city'" :name="form.order.direction === 'asc' ? 'arrow-up' : 'arrow-down'"/>
            </div>
          </th>
...
export default {
  ...
  methods: {
    orderBy(field) {
      this.form.order.field = field
      this.form.order.direction = this.form.order.direction === 'asc' ? 'desc' : 'asc'
    },
    reset() {
